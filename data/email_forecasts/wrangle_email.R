...
extract_data_from_email <- function(email_file) {
  ...
  # Extracting email sent time
  email_time_str <- email[grep("^Date:", email)]
  email_time_utc <- strptime(sub("^Date: ", "", email_time_str), "%a, %d %b %Y %H:%M:%S %z", tz="UTC")
  email_time_cst <- with_tz(email_time_utc, tzone = "CST6CDT")  # CST without daylight saving
  
  # Determine if the email was sent in the morning or afternoon
  email_period <- ifelse(hour(email_time_cst) < 12, "morning", "afternoon")
  
  # Initiate empty vector and boolean variable
  v <-  c()
  b <- FALSE
  
  # Initialize data frame with 12 columns
  ...
  # "$$" or "NATIONAL TEMPERATURE EXTREMES"
  for (line in email) {
  ...
  for (line in v) {
    if (str_detect(line, "\\.\\.|FORECAST|PCPN")) { }
      ...
      # Split once on "MM" (missing value) or first number to retrieve city
      ...
      row <- c(date_and_time, city, previous_lo, previous_hi, previous_precip,
               today_lo, today_hi,  today_outlook, 
               tomorrow_lo, tomorrow_hi, tomorrow_outlook, email_period)
      df[nrow(df) + 1, ] <- row
    }
  }
...
process_emails_to_csv <- function(directory) {
  email_files <- list.files(directory, pattern = "*.eml", full.names = TRUE)
  
  for (email_file in email_files) {
    df <- extract_data_from_email(email_file)
    csv_file_name <- paste0(sub("\\.eml", "", basename(email_file)), ".csv")
    write.csv(df, file.path(output_directory, csv_file_name), row.names = FALSE)
  }
}

# Call the function and specify the directory containing the email files
# Change directory to to the email_data
directory <- "/Weather/R_code_Test/data/email_data"
output_directory <- "/Weather/R_code_Test/data/output_csv"
process_emails_to_csv(directory)
